...
gdal.AllRegister()
# open the image
ds = gdal.Open(r'E:\Runoff\runoff_14_r30_PrXY_redDeer.tif')
if ds is None:
...
ofile  = open('annualRunoff.csv', "wb")
writer = csv.writer(ofile)
header = ['SPARROWID','COUNT','CellAreaSQM','CatchmentAreaSQM','SUM(mm/year)','SUM(m/year)','R1(m3/year)','R2(m3/year)','Error','R1(m3/s)','R2_Chosen(m3/s)']
...
while feature:
	...
	runoffAveM = np.sum(dataInPoly)/(1000*ncell)
	runoffAveCell = ncell*cellArea*runoffAveM
	runoffAveArea = feature.GetField('Shape_Area') * runoffAveM
	
	print 'Ave. runoff m3/s : ' ,j+1, runoffAveCell/(60*60*24*365),  runoffAveArea/(60*60*24*365)
	...
	writer.writerow([int(j),\
					int(ncell),\
					float(ncell*cellArea),\
					float(feature.GetField('Shape_Area')),\
					float(np.sum(dataInPoly)),\
					float(np.sum(dataInPoly)/1000),\
					float(runoffAveCell),\
					float(runoffAveArea),\
					float( 100*(runoffAveCell-runoffAveArea)/runoffAveCell ),\
					float(runoffAveCell/(60*60*24*365)),\
					float(runoffAveArea/(60*60*24*365))])
	sumError = sumError + abs(100*(runoffAveCell-runoffAveArea)/runoffAveCell)
...
print 'Ave error of streamflow estimation: ', sumError/j, ' %'
# Cleaning up memory
band = None
data = None	